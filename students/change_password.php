...
if (count($_POST) > 0) {
    $result = mysqli_query($con, "SELECT *from `new_admission` WHERE email='" . $_SESSION["email"] . "'");
    $row = mysqli_fetch_array($result);
    if ($_POST["currentPassword"] == $row["password"]) {
        mysqli_query($con, "UPDATE `new_admission` set password='" . $_POST["newPassword"] . "' WHERE email='" . $_SESSION["email"] . "'");
